	...
	useEffect(() => {
		const fetchProduct = () => {
			...
			try {
				...
				if (!foundProduct) {
					console.error(
						`404 Error: User attempted to access non-existent route: /products/${id}`
					);
					navigate("/products");
					return;
				}
	...
	const productSections = [
		{
			...
			content:
				"تم استخدام مجموعة من المواد عالية الجودة في صناعة هذا المنتج، بما في ذلك النسيج الطبيعي والخشب المعالج والصبغات الطبيعية المستخرجة من أعشاب محلية.",
		},
		{
			id: "craftsmanship",
			title: "الحرفية والصناعة",
			content:
				"صنع هذا المنتج بعناية فائقة من قبل حرفيين ذوي خبرة تمتد لعقود، باستخدام تقنيات تقليدية متوارثة عبر الأجيال مع الحفاظ على الأصالة والدقة في التفاصيل.",
		},
		{
			id: "history",
			title: "التاريخ والأصل",
			content:
				"ينبع تصميم هذا المنتج من تقاليد فنية عريقة تعود لمئات السنين، وتعكس الزخارف والنقوش المستخدمة تراث المنطقة وثقافتها الغنية بالتفاصيل الفنية الدقيقة.",
		},
	];
						...
						<Tabs
							...
							<TabsContent value='overview'>
								<motion.div
									...
									className='space-y-4'>
									<p className='mb-4'>
										هذا المنتج مصنوع يدويًا بواسطة حرفيين مهرة باستخدام تقنيات
										تقليدية متوارثة عبر الأجيال. يتميز بتفاصيل دقيقة وجودة عالية
										تعكس روح الحرف اليدوية العربية الأصيلة.
									</p>
